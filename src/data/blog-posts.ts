export interface BlogPost {
  id: number
  slug: string
  title: string
  excerpt: string
  content: string
  date: string
  category: string
  readTime: string
  featured: boolean
  keywords: string[]
  metaDescription: string
  author: string
}

export const blogPosts: BlogPost[] = [
  {
    id: 1,
    slug: 'las-vegas-real-estate-market-update-november-2025',
    title: 'Las Vegas Real Estate Market Update - November 2025',
    excerpt:
      'Current market trends, pricing insights, and what to expect in the Las Vegas housing market this November. Analysis of Summerlin, Henderson, and North Las Vegas neighborhoods.',
    content: `
# Las Vegas Real Estate Market Update - November 2025

The Las Vegas real estate market continues to show resilience in November 2025, with steady activity across all price ranges and neighborhoods throughout Las Vegas Valley.

## Market Overview

As of November 2025, Las Vegas Valley has experienced moderate price appreciation with increased inventory providing more options for buyers. The market has normalized from rapid growth seen in 2021-2022, with steady appreciation continuing in premium areas like Summerlin (zip codes 89134-89144) and Henderson's Green Valley (89012, 89014).

## Key Market Indicators

**Inventory Levels**: Increased compared to previous years, giving buyers more options across zip codes 89103-89148 (Las Vegas), 89002-89074 (Henderson), and 89030-89086 (North Las Vegas).

**Price Trends**: 
- **Summerlin** (89134-89144): Average home prices $600,000-$800,000, with luxury properties in The Ridges exceeding $1M
- **Henderson** (89002-89015): Average prices $500,000-$700,000, with Green Valley and Seven Hills commanding premiums
- **North Las Vegas** (89030-89086): More affordable options $350,000-$550,000, with newer developments like Skye Canyon (89084) and Centennial Hills (89031) gaining popularity
- **Spring Valley** (89103-89147): Diverse pricing $400,000-$600,000

**Days on Market**: Average 30-45 days for well-priced properties, faster in desirable neighborhoods.

## Neighborhood Spotlight

### Summerlin Real Estate Market
Summerlin continues to be the premier master-planned community in Las Vegas Valley, with strong demand for homes in zip codes 89134, 89135, 89138, and 89144. The Ridges and other premium communities maintain strong values.

### Henderson Market Trends
Henderson's family-friendly environment attracts buyers to zip codes 89002-89015, with Green Valley (89012, 89014) and Seven Hills (89052, 89074) seeing consistent demand.

### North Las Vegas Growth
North Las Vegas (89030-89086) offers excellent value with newer construction in Skye Canyon (89084) and Centennial Hills (89031), attracting first-time buyers and families.

## What This Means for Buyers and Sellers

**For Buyers**: More inventory means more negotiating power, especially in certain price ranges. Take time to explore different neighborhoods from The Ridges to Skye Canyon.

**For Sellers**: Proper pricing and staging remain critical. Homes in premium areas like Lake Las Vegas or Macdonald Highlands continue to attract qualified buyers.

## Looking Ahead

Market fundamentals remain strong with population growth, diverse job market, and relative affordability compared to California markets. Working with an experienced agent like Dr. Jan Duffy ensures you understand current market conditions specific to your neighborhood.

Contact Dr. Jan Duffy at (702) 200-3422 for a personalized market analysis of your Las Vegas Valley neighborhood.
    `,
    date: 'November 2, 2025',
    category: 'Market Updates',
    readTime: '6 min read',
    featured: true,
    keywords: [
      'Las Vegas real estate market',
      'November 2025 market trends',
      'Summerlin real estate',
      'Henderson housing market',
      'Las Vegas home prices',
      'real estate market analysis',
    ],
    metaDescription:
      'Current Las Vegas real estate market trends for November 2025. Analysis of Summerlin, Henderson, and North Las Vegas neighborhoods with pricing insights and market indicators.',
    author: 'Dr. Jan Duffy',
  },
  {
    id: 2,
    slug: 'first-time-homebuyer-guide-las-vegas',
    title: 'First-Time Homebuyer Guide: Las Vegas Edition',
    excerpt:
      'Everything you need to know about buying your first home in Las Vegas, from pre-approval to closing. Complete guide for first-time buyers with neighborhood recommendations.',
    content: `
# First-Time Homebuyer Guide: Las Vegas Edition

Buying your first home in Las Vegas is an exciting journey! This comprehensive guide walks you through every step, from getting pre-approved to closing on your dream home.

## Step 1: Get Pre-Approved

Before you start looking at homes, get pre-approved for a mortgage. This tells you exactly how much you can afford and strengthens your offers.

**First-Time Buyer Programs Available**:
- **Home Is Possible (HIP)**: Down payment assistance up to 5% of loan amount
- **FHA Loans**: 3.5% down payment (vs. typical 20%)
- **VA Loans**: No down payment for eligible veterans
- **USDA Loans**: Available for rural areas (some North Las Vegas areas qualify)

## Step 2: Understand Your Budget

In Las Vegas, factor in:
- **Housing**: Mortgage payment (aim for 30% of gross income)
- **HOA Fees**: $50-$600/month depending on community (Summerlin, Henderson, etc.)
- **Property Taxes**: Low in Nevada (0.6%-1.2% of assessed value)
- **Utilities**: Higher in summer due to air conditioning
- **Insurance**: Homeowner's insurance costs

## Step 3: Choose the Right Neighborhood

Las Vegas Valley offers diverse neighborhoods for first-time buyers:

### Best Neighborhoods for First-Time Buyers:

**North Las Vegas** (zip codes 89030-89086):
- More affordable entry-level homes ($350,000-$550,000)
- Newer construction in Skye Canyon (89084) and Centennial Hills (89031)
- Growing community amenities

**Spring Valley** (89103-89147):
- Established neighborhoods with good value
- Diverse housing options
- Central location

**Henderson** (89002-89015):
- Some affordable areas for first-time buyers
- Excellent schools in Green Valley (89012, 89014)
- Family-friendly communities

**Enterprise** (89118-89123):
- Newer developments like Mountain's Edge (89178)
- Good value for first-time buyers

## Step 4: Find the Right Home

Use our advanced search tool to filter by:
- Price range
- Location (specific zip codes)
- Number of bedrooms/bathrooms
- Property type (single-family, condo)

Attend open houses on weekends to explore different neighborhoods from Summerlin to North Las Vegas.

## Step 5: Make an Offer

Work with your agent (like Dr. Jan Duffy) to:
- Research comparable sales in your area
- Determine fair market value
- Craft a competitive offer
- Include appropriate contingencies

## Step 6: Home Inspection

Always get a home inspection, especially important in Las Vegas for:
- HVAC systems (critical in desert climate)
- Roof condition (intense sun exposure)
- Plumbing (hard water considerations)
- Foundation (desert soil conditions)

## Step 7: Closing Process

Your agent will guide you through:
- Final walkthrough
- Closing documents
- Key transfer
- Move-in coordination

## Tips for First-Time Buyers

1. **View Multiple Properties**: See at least 3-5 homes before making an offer
2. **Factor in Total Cost**: Don't just focus on purchase price - consider HOA, taxes, utilities
3. **Work with a Buyer's Agent**: Dr. Jan Duffy's buyer services are typically free (seller pays commission)
4. **Explore First-Time Buyer Programs**: Take advantage of available assistance
5. **Consider Future Needs**: Think about schools, commute, and long-term plans

## Ready to Start Your Search?

Contact Dr. Jan Duffy at (702) 200-3422 for personalized guidance on buying your first home in Las Vegas. She specializes in working with first-time buyers and can help you navigate the entire process.
    `,
    date: 'November 1, 2025',
    category: 'Buyer Tips',
    readTime: '10 min read',
    featured: false,
    keywords: [
      'first-time homebuyer Las Vegas',
      'buy first home Las Vegas',
      'first-time buyer guide',
      'Las Vegas home buying',
      'first-time buyer programs Las Vegas',
      'affordable neighborhoods Las Vegas',
    ],
    metaDescription:
      'Complete guide to buying your first home in Las Vegas. Includes pre-approval, neighborhood recommendations, first-time buyer programs, and step-by-step process.',
    author: 'Dr. Jan Duffy',
  },
  {
    id: 3,
    slug: 'top-5-neighborhoods-families-las-vegas',
    title: 'Top 5 Neighborhoods for Families in Las Vegas',
    excerpt:
      'Discover the best family-friendly neighborhoods in Las Vegas with great schools, safety, and amenities. Comprehensive guide to Summerlin, Henderson, Green Valley, and more.',
    content: `
# Top 5 Neighborhoods for Families in Las Vegas

Finding the perfect neighborhood for your family involves considering schools, safety, amenities, and overall quality of life. Here are the top 5 family-friendly neighborhoods in Las Vegas Valley.

## 1. Summerlin (Zip Codes 89134, 89135, 89138, 89144)

**Why Families Love Summerlin**:
- **Excellent Schools**: Top-rated public and private schools (many rated 8-10/10)
- **Master-Planned Community**: Downtown Summerlin shopping, parks, golf courses
- **Safety**: Low crime rates, gated communities available
- **Amenities**: Parks, trails, community centers, Red Rock Canyon access
- **Quality of Life**: Beautiful mountain views, well-maintained neighborhoods

**Price Range**: $400,000-$2M+ (luxury in The Ridges)

**Best For**: Families seeking premium living with excellent schools and amenities

## 2. Henderson - Green Valley (Zip Codes 89012, 89014)

**Why Families Love Green Valley**:
- **Family-Friendly**: Consistently ranked among safest areas
- **Great Schools**: Highly-rated schools serving the community
- **Shopping**: Green Valley Ranch with extensive amenities
- **Parks & Recreation**: Excellent community parks and facilities
- **Community Feel**: Strong neighborhood involvement

**Price Range**: $500,000-$800,000

**Best For**: Families wanting established, safe communities with great schools

## 3. Henderson - Seven Hills (Zip Codes 89052, 89074)

**Why Families Love Seven Hills**:
- **Gated Community**: Additional security and exclusivity
- **Excellent Schools**: Top-performing schools in the area
- **Safety**: One of Henderson's safest communities
- **Luxury Living**: Upscale homes and amenities
- **Mountain Views**: Beautiful scenery

**Price Range**: $600,000-$1.5M

**Best For**: Families seeking upscale, secure communities

## 4. Henderson - Anthem (Zip Code 89044)

**Why Families Love Anthem**:
- **Gated Community**: Strong security features
- **Newer Construction**: Modern homes with contemporary features
- **Community Amenities**: Parks, pools, community centers
- **Good Schools**: Strong educational options
- **Family-Oriented**: Designed with families in mind

**Price Range**: $500,000-$900,000

**Best For**: Families wanting newer homes in secure communities

## 5. North Las Vegas - Centennial Hills/Skye Canyon (Zip Codes 89031, 89084)

**Why Families Love This Area**:
- **Affordability**: More affordable than Summerlin or Henderson
- **New Construction**: Modern homes with energy-efficient features
- **Growing Communities**: Expanding amenities and infrastructure
- **Good Schools**: Improving school ratings with new facilities
- **Family-Friendly**: Community-focused developments

**Price Range**: $350,000-$600,000

**Best For**: Families seeking newer homes at more affordable prices

## Key Factors for Families

### School Quality
Top-rated school districts include:
- Summerlin schools (zip codes 89134-89144)
- Henderson Green Valley schools (89012, 89014)
- Seven Hills schools (89052, 89074)

### Safety
All listed neighborhoods have low crime rates and active neighborhood watch programs.

### Amenities
Look for:
- Parks and playgrounds
- Community centers
- Shopping and dining
- Healthcare facilities
- Family-friendly activities

### Commute
Consider proximity to:
- Employment centers
- Las Vegas Strip (for work)
- Major highways (I-15, US-95, I-215)

## Making Your Decision

When choosing a neighborhood:
1. **Visit Multiple Areas**: Spend time in different neighborhoods
2. **Check School Ratings**: Research specific schools
3. **Consider Long-Term**: Think about resale value and future growth
4. **Talk to Residents**: Ask current families about their experience
5. **Work with a Local Expert**: Dr. Jan Duffy knows all Las Vegas neighborhoods

## Need Help Choosing?

Dr. Jan Duffy specializes in helping families find the perfect neighborhood in Las Vegas Valley. Contact her at (702) 200-3422 for personalized neighborhood recommendations based on your family's needs, budget, and priorities.
    `,
    date: 'October 28, 2025',
    category: 'Neighborhoods',
    readTime: '8 min read',
    featured: false,
    keywords: [
      'best neighborhoods for families Las Vegas',
      'family-friendly Las Vegas neighborhoods',
      'best schools Las Vegas',
      'safe neighborhoods Las Vegas',
      'Summerlin families',
      'Henderson families',
    ],
    metaDescription:
      'Top 5 family-friendly neighborhoods in Las Vegas with excellent schools, safety, and amenities. Guide to Summerlin, Henderson Green Valley, Seven Hills, Anthem, and more.',
    author: 'Dr. Jan Duffy',
  },
  {
    id: 4,
    slug: 'selling-your-home-las-vegas-pricing-strategy',
    title: 'Selling Your Home in Las Vegas: Pricing Strategy',
    excerpt:
      'Learn how to price your home competitively in the Las Vegas market for a quick and profitable sale. Expert pricing strategies from Open House Expert Dr. Jan Duffy.',
    content: `
# Selling Your Home in Las Vegas: Pricing Strategy

Pricing your Las Vegas home correctly is the most critical factor in achieving a quick sale and maximizing your return. Here's how to develop the right pricing strategy for November 2025.

## The Importance of Correct Pricing

**Overpricing Consequences**:
- Extended days on market
- Lower final sale price (buyers see stale listings)
- Reduced buyer interest
- Negative market perception

**Correct Pricing Benefits**:
- Faster sale (30-45 days average)
- Multiple offers in competitive markets
- Higher final sale price
- Smooth transaction process

## Market Analysis is Key

Dr. Jan Duffy provides comprehensive market analysis including:

### Comparable Sales (Comps)
- Recent sales of similar properties in your neighborhood
- Properties sold within last 3-6 months
- Similar size, condition, and features
- Same zip code and neighborhood (Summerlin 89134-89144, Henderson 89002-89015, etc.)

### Active Listings
- Current competition in your price range
- Days on market for similar properties
- Listing prices vs. final sale prices
- Buyer activity levels

### Market Trends
- Price per square foot trends
- Inventory levels in your area
- Buyer demand by price range
- Seasonal patterns (November is typically active)

## Pricing by Neighborhood

### Summerlin (Zip Codes 89134-89144)
- **Entry-level**: $400,000-$600,000
- **Mid-range**: $600,000-$1,000,000
- **Luxury** (The Ridges): $1M-$2M+
- **Strategy**: Premium areas hold value well, price competitively within range

### Henderson (89002-89015)
- **Green Valley** (89012, 89014): $500,000-$800,000
- **Seven Hills** (89052, 89074): $600,000-$1.5M
- **General**: $400,000-$700,000
- **Strategy**: Family-friendly areas attract buyers, price at market value

### North Las Vegas (89030-89086)
- **Skye Canyon** (89084): $400,000-$600,000
- **Centennial Hills** (89031): $350,000-$550,000
- **General**: $350,000-$550,000
- **Strategy**: Value-focused market, price competitively for quick sale

### Spring Valley (89103-89147)
- **General**: $400,000-$600,000
- **Strategy**: Diverse market, focus on value and condition

## Pricing Strategies

### 1. Competitive Pricing
Price slightly below market value to attract multiple offers. Works well in:
- Active markets
- Popular neighborhoods (Summerlin, Henderson)
- Entry to mid-range price points

### 2. Market Value Pricing
Price at current market value. Ideal when:
- Market is balanced
- Property is in good condition
- Neighborhood has steady demand

### 3. Premium Pricing
Price above market for unique properties. Only use when:
- Exceptional condition or features
- Unique location (The Ridges, Lake Las Vegas)
- Luxury market with fewer comps

## Factors Affecting Price

### Property Condition
- **Turn-key**: Price at top of range
- **Needs updates**: Price 5-10% below market
- **Major repairs needed**: Significant price reduction required

### Location Factors
- **Desirable neighborhoods** (The Ridges, Green Valley): Premium pricing
- **Growing areas** (Skye Canyon, Centennial Hills): Competitive pricing
- **Established areas**: Market value pricing

### Market Timing
- **November 2025**: Active market, price competitively
- **Peak season** (Spring): Can price slightly higher
- **Slower periods**: Price more aggressively

## Common Pricing Mistakes

1. **Emotional Pricing**: Don't let personal attachment inflate price
2. **Ignoring Condition**: Price must reflect actual condition
3. **Not Updating**: If not selling, adjust price after 30 days
4. **Comparing to Old Sales**: Use recent comps (last 3-6 months)
5. **Ignoring Competition**: Monitor active listings in your area

## Get Your Home Valuation

Dr. Jan Duffy provides free, comprehensive home valuations including:
- Detailed market analysis
- Comparable sales review
- Pricing recommendations
- Neighborhood market trends
- Strategic pricing advice

## Ready to Price Your Home?

Contact Dr. Jan Duffy at (702) 200-3422 for a professional market analysis and pricing strategy for your Las Vegas Valley home. Her expertise in Summerlin, Henderson, North Las Vegas, and all Las Vegas neighborhoods ensures you price correctly for maximum return.
    `,
    date: 'October 25, 2025',
    category: 'Seller Tips',
    readTime: '9 min read',
    featured: false,
    keywords: [
      'selling home Las Vegas',
      'home pricing strategy',
      'Las Vegas home values',
      'price home to sell',
      'market analysis Las Vegas',
      'home selling tips',
    ],
    metaDescription:
      'Expert pricing strategy for selling your home in Las Vegas. Learn how to price competitively for quick sale and maximum return with neighborhood-specific guidance.',
    author: 'Dr. Jan Duffy',
  },
  {
    id: 5,
    slug: 'investment-properties-las-vegas-2025-outlook',
    title: 'Investment Properties in Las Vegas: 2025 Outlook',
    excerpt:
      'Analysis of rental market trends and investment opportunities in the Las Vegas area. ROI analysis, best neighborhoods for investors, and market outlook for 2025.',
    content: `
# Investment Properties in Las Vegas: 2025 Outlook

Las Vegas continues to be an attractive market for real estate investors, with strong rental demand, population growth, and relative affordability compared to coastal markets.

## Market Overview for Investors

As of November 2025, Las Vegas investment property market offers:
- **Strong Rental Demand**: Population growth drives rental needs
- **Affordable Entry Points**: Lower prices than California markets
- **Positive Cash Flow Potential**: In many areas with proper analysis
- **Appreciation Potential**: Long-term growth outlook remains positive
- **Diverse Opportunities**: From single-family to condos to multi-unit

## Best Neighborhoods for Investment

### 1. North Las Vegas (Zip Codes 89030-89086)
**Investment Appeal**:
- Lower entry prices ($300,000-$500,000)
- Growing population and job market
- New construction in Skye Canyon (89084), Centennial Hills (89031)
- Strong rental demand
- Potential for appreciation

**ROI Considerations**: Good cash flow potential, newer properties require less maintenance

### 2. Spring Valley (Zip Codes 89103-89147)
**Investment Appeal**:
- Established neighborhoods with steady demand
- Diverse housing options
- Central location
- Affordable entry points
- Consistent rental market

**ROI Considerations**: Stable returns, established area with predictable performance

### 3. Henderson (Some Areas - 89002, 89015)
**Investment Appeal**:
- More affordable than premium Henderson areas
- Growing rental market
- Family-friendly attracts long-term tenants
- Potential for appreciation

**ROI Considerations**: Moderate cash flow, good for long-term appreciation strategy

### 4. Enterprise (Zip Codes 89118-89123)
**Investment Appeal**:
- Newer developments like Mountain's Edge (89178)
- Growing community
- Affordable options
- Potential for growth

**ROI Considerations**: Growth potential, newer properties with modern features

## Investment Property Types

### Single-Family Homes
**Pros**: 
- Strong appreciation potential
- Attractive to families (long-term tenants)
- Easier financing

**Best For**: Long-term buy-and-hold investors

**Areas**: North Las Vegas (89030-89086), Spring Valley (89103-89147)

### Condos
**Pros**:
- Lower entry price
- HOA handles exterior maintenance
- Attractive to renters seeking amenities

**Considerations**:
- HOA fees reduce cash flow
- Appreciation may be slower

**Areas**: Summerlin (89134-89144), Henderson (89002-89015), near Strip

### Multi-Unit Properties
**Pros**:
- Multiple income streams
- Economies of scale
- Strong cash flow potential

**Considerations**:
- Higher initial investment
- More management required

**Areas**: Central Las Vegas, established neighborhoods

## ROI Analysis Factors

### Cash Flow
- **Rental Income**: Research market rents in your target area
- **Expenses**: Mortgage, taxes, insurance, HOA, maintenance, property management
- **Net Cash Flow**: Income minus expenses

### Appreciation
- **Historical Trends**: Las Vegas has shown long-term appreciation
- **Market Fundamentals**: Population growth, job market, limited land
- **Neighborhood Growth**: Emerging areas offer higher appreciation potential

### Tax Benefits
- **Depreciation**: Write-off property value over time
- **Deductions**: Interest, repairs, maintenance, travel
- **1031 Exchange**: Defer capital gains by reinvesting

## Market Trends for Investors

**November 2025 Outlook**:
- Increased inventory provides more buying opportunities
- Rental rates remain strong due to population growth
- Interest rates impact financing costs
- Market normalization allows for strategic purchases

**Long-Term Outlook**:
- Continued population growth supports rental demand
- Diverse economy (tourism, tech, healthcare) provides stability
- Limited land availability supports appreciation
- Relative affordability compared to California

## Investment Strategies

### 1. Buy and Hold
- Long-term appreciation focus
- Steady rental income
- Areas: All Las Vegas Valley neighborhoods
- Best for: Long-term wealth building

### 2. Fix and Flip
- Quick turnaround
- Requires renovation expertise
- Areas: Older neighborhoods needing updates
- Best for: Experienced investors with capital

### 3. Short-Term Rentals
- Higher income potential
- Requires active management
- Areas: Near Strip, tourist areas
- **Important**: Check local regulations

### 4. Buy, Rehab, Rent, Refinance (BRRRR)
- Recycle capital
- Build portfolio faster
- Areas: Properties needing updates
- Best for: Experienced investors

## Key Investment Considerations

### Location Analysis
- **Job Growth**: Areas near employment centers
- **School Quality**: Affects tenant quality and property values
- **Infrastructure**: New development and improvements
- **Crime Rates**: Safety affects desirability

### Property Analysis
- **Condition**: Factor in renovation costs
- **Age**: Newer = less maintenance initially
- **Features**: What tenants want (AC, updated kitchens, etc.)
- **HOA**: Fees impact cash flow

### Financial Analysis
- **Down Payment**: 20-25% typically required for investment properties
- **Interest Rates**: Impact monthly costs
- **Reserves**: Maintain 6 months expenses
- **Return Metrics**: Cap rate, cash-on-cash return, IRR

## Las Vegas Investment Advantages

1. **No State Income Tax**: Increases investment returns
2. **Low Property Taxes**: Lower than many states
3. **Population Growth**: Drives rental demand
4. **Diverse Economy**: Beyond tourism
5. **Affordability**: Accessible entry points
6. **Landlord-Friendly Laws**: Reasonable regulations

## Working with an Investment-Savvy Agent

Dr. Jan Duffy understands investment property dynamics across Las Vegas Valley. She can help:
- Identify investment opportunities
- Analyze cash flow and ROI
- Connect you with property managers
- Navigate investment property financing
- Understand neighborhood investment potential

## Ready to Invest?

Contact Dr. Jan Duffy at (702) 200-3422 for investment property analysis and opportunities in Las Vegas Valley neighborhoods from North Las Vegas to Summerlin, from Skye Canyon to Green Valley.
    `,
    date: 'October 20, 2025',
    category: 'Investment',
    readTime: '12 min read',
    featured: false,
    keywords: [
      'Las Vegas investment properties',
      'real estate investing Las Vegas',
      'rental properties Las Vegas',
      'investment opportunities Las Vegas',
      'buy investment property Las Vegas',
      'Las Vegas rental market',
    ],
    metaDescription:
      'Comprehensive guide to investment properties in Las Vegas for 2025. Analysis of rental market, best neighborhoods, ROI factors, and investment strategies.',
    author: 'Dr. Jan Duffy',
  },
]

export const getBlogPostBySlug = (slug: string): BlogPost | undefined => {
  return blogPosts.find((post) => post.slug === slug)
}

export const getFeaturedPosts = (): BlogPost[] => {
  return blogPosts.filter((post) => post.featured)
}

export const getPostsByCategory = (category: string): BlogPost[] => {
  return blogPosts.filter((post) => post.category === category)
}

export const getRecentPosts = (limit = 5): BlogPost[] => {
  return blogPosts
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    .slice(0, limit)
}

